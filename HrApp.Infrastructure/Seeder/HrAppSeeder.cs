...
public class HrAppSeeder(HrAppContext dbContext) : IHrAppSeeder
{
    ...
    public async Task Seed()
    {
        ...
        if (!_dbContext.Department.Any())
        {
            ...
            team.Name = "Druzyna 1";
            departments[0].Teams = [team];
            departments[0].HeadOfDepartment = users.FirstOrDefault(x => x.Roles.Any(y => y.Name == Roles.Ceo.ToString()))!;
    ...
    private List<Team> GetTeams(List<User> users, List<Department> departments)
    {

        var teams = new Faker<Team>(Locale)
            .RuleFor(x => x.Name, y => $"Zespół {y.Commerce.Department()}")
            .RuleFor(x => x.TeamLeader, y => y.PickRandom(users))
            .RuleFor(x => x.TeamLeaderId, (y, x) => x.TeamLeader.Id)
            .RuleFor(x => x.Employers, y => y.PickRandom(users, y.Random.Int(3, 8)).ToList())
            .RuleFor(x => x.Department, y => y.PickRandom(departments))
            .RuleFor(x => x.DepartmentId, (y, x) => x.Department.Id)
            .RuleFor(x => x.Assignments, _ => new List<Assignment>())
            .RuleFor(x => x.AnonymousFeedbacks, _ => new List<AnonymousFeedback>())
            .RuleFor(x => x.Offers, _ => new List<Offer>())
            .Generate(30);
